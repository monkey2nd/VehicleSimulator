...
def get_data(seed_ls, penetration, car_max, source_path, data_set: List[List], num_c, num_p) -> List[List]:
    """
    減速量に関係するデータを取得する関数
    """
    data = []
    for seed in seed_ls:
        data_tmp = []
        path = source_path + "/普及率" + str(penetration * 100) + "%" + "/車両数" + str(car_max + 50) + "/seed" + str(
            seed) + ".xlsx"
...
def get_table(col_title_ls, seed_ls, car_max, penetration, data_set, num_c, num_p, source_path):
    ...
    datas = get_data(seed_ls=seed_ls, penetration=penetration, car_max=car_max, source_path=source_path,
                     data_set=data_set, num_p=num_p, num_c=num_c)
...
def get_table2(ws: Worksheet, data_set, penetration_ls, car_max_ls, col_title_ls):
    for car_max_index in range(len(car_max_ls)):
        ...
        write_list(ws=ws, datas=template, column=(len(col_title_ls) + 3) * len(penetration_ls) + 1,
